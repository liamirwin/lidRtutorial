...
Forests are __highly__ variable! This means that some algorithms and parameters will work better than others depending on the data you have. Play around with algorithms and see which works best for your data.
:::
...
We can map any user-defined function at the tree level using the `crown_metrics()` function, just like `grid_metrics()`. Here, we calculate the convex hull area of each tree using a custom function `f()` and then visualize the results.
